...
| [01-09 equality-locus calculus](01-geometry/09-equality-locus-and-intersection-calculus.md) | shipped | Constraint manifold, not a PDE solver; `branch` / `condition` / `converged`; G1–G6 CI |
...
| [01-13 operator family](01-geometry/13-operator-family.md) | shipped | Scan of the six roles; catalog + rejects; not a seventh `op`; G1–G5 document gates in CI; first spend `BiasScan(op="integral")` + 09-14 both shipped |
...
| [02-03 jet-KAN](02-architectures/03-jet-kan-univariate-basis.md) | shipped | Edge-wise univariate bases; exactness of the model jet, not the target; KA theorem does not justify; G1/G3/G4/G5 CI; G2 cost **leftover-recorded** (leftover #41; ~2.2x vs autodiff, need 5x), not CI `all_passed` |
...
| [02-07 hierarchical pack tree](02-architectures/07-hierarchical-pack-tree-fmm.md) | shipped | 1-D offsets; `eta=0` bit-identical to dense; G1–G5 CI; G2 bound **earned**; G3 complexity **earned** (leftover #13 closed; cached `O(p)` multipole, dense crossover); G4 `separation_for_accuracy` **earned**; G5 parity **earned** |
| [02-08 equivariant and manifold scan](02-architectures/08-equivariant-and-manifold-scan.md) | shipped | Gaussian-family steering only; discrete `C_L`, not SO(2)/SO(3); G1–G4 **earned**; G5 anisotropic-interface **leftover-recorded** (leftover #25; `--full`, no task loop); orbit cost **leftover-recorded** (leftover #43), not in CI `all_passed` |
| [02-09 soliton tanh-method nets](02-architectures/09-soliton-tanh-method-networks.md) | shipped | Tanh algebra, not a collapse; multi-kink is not the n-soliton formula; G1/G2/G3/G5 **earned**; G4 PINN init-win **leftover-recorded** (leftover #22; `--full`, no train); algebraic cost **leftover-recorded** (leftover #44), not in CI `all_passed` |
| [02-10 Hermite ladder nets](02-architectures/10-hermite-ladder-oscillator-net.md) | shipped | Raw tower is not the QHO eigenbasis; Rodrigues reweight required; G1–G3/G6 **earned**; G4 FermiNet many-body **leftover-recorded** (leftover #21; `--full`); exact-ladder vs FD **leftover-recorded** (leftover #45); G5 anharmonic **leftover-recorded** (leftover #26; loses to FD grid), not in CI `all_passed` |
| [02-11 transfer-matrix layered media](02-architectures/11-transfer-matrix-layered-media.md) | shipped | 1-D ABCD; `continuum_claim=False`; distinct from `geometry.gauge.transfer`; G1–G3/G6 **earned**; G4 inverse-design **leftover-recorded** (leftover #23; `--full`, no optimizer); stack cost **leftover-recorded** (leftover #46); G5 conservation **leftover-recorded** (leftover #27; no MLP surrogate), not in CI `all_passed` |
| [02-12 equality-intersection nets](02-architectures/12-equality-intersection-ansatz-nets.md) | shipped | Layer on 01-09; always `branch` / `condition` / `converged`; not a general PDE solver; G1–G3/G5/G6 **earned**; G4 Burgers RH **leftover-recorded** (leftover #29; clean speed from `affine_locus`; noisy contour stays `--full`), not in CI `all_passed` |
| [02-13 linearizing transforms](02-architectures/13-linearizing-transform-layers.md) | shipped | Named Cole-Hopf / Miura / Bäcklund / Darboux; exactness to jet order N; G1 jet identity CI; G6 torch/jax parity CI; G2 n-soliton **leftover-recorded** (leftover #51); G3 Burgers **leftover-recorded** (leftover #39); G4 permutability **leftover-recorded** (leftover #52), not in CI `all_passed`; 03-11 search stays designed |
...
| [07-09 similarity profile](07-frontier/09-similarity-profile-axis-germ.md) | shipped | Lemma 4.1 operators + axis germ + source jets; G1–G5 CI; founding bias collapse, not temperature collapse; not a forced-blowup reproof |
...
| [07-18 unforced BKM slab](07-frontier/18-unforced-bkm-slab.md) | shipped | Force-free 2-D TG slab + Interval BKM integral + 07-02 weak residual; G1–G5 CI; not Clay A/B |
...
| [09-02 jet-token transformer](09-inventions/02-jet-token-transformer.md) | shipped | `compose_jet` mix, not softmax-of-values; G1–G4 CI; model jet, not ImageNet; founding bias collapse, not temperature collapse |
| [09-03 FTC-Net](09-inventions/03-ftc-net.md) | shipped | Integral cell + collapse head; G1–G4 CI; not a VPINN; founding bias collapse, not temperature collapse |
...
| [09-13 jet-Hopfield](09-inventions/13-jet-hopfield.md) | shipped | Memories are germs; G1–G4 CI; contact match, not vector Hopfield |
| [09-14 integral-kernel operator](09-inventions/14-integral-kernel-operator.md) | shipped | Volumetric DeepONet kernel is an OMBU `integral`; G1–G4 CI; not BEM-Net; founding bias collapse, not temperature collapse |
| [09-15 q-OMBU / timescale](09-inventions/15-q-ombu-timescale.md) | shipped | Hybrid q / Hilger layers; G1–G4 CI; named `q -> 1` / `mu -> 0`, not a continuum PDE |
...
| [09-20 homotopy continuation](09-inventions/20-homotopy-continuation.md) | shipped | Path of problems; each step 08-04-accepted; G1–G4 CI; empty ball is a halt |
| [09-21 exact score matching](09-inventions/21-exact-score-matching.md) | shipped | Hyvärinen on an OMBU score; G1–G4 CI; CNF exact `div` is prior art |
| [09-22 inverse-design](09-inventions/22-inverse-design.md) | shipped | Newton-on-`x` with exact `sigma'`; G1–G4 CI; not 08-03 layer invert |
| [09-23 sharpness regularizer](09-inventions/23-sharpness-regularizer.md) | shipped | Exact `lambda_max` / `Tr(H)` in the loss; G1–G4 CI; not 08-06 schedule |
