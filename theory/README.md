...
| [01-03 arrangement geometry](01-geometry/03-hyperplane-arrangement-geometry.md) | gated | `omnibias.partition.arrangement`; temperature collapse, sampled subgraph, sound gap not P vs NP; G1–G4 CI; cost vs `n`/`D` **reported** (cutoff `n<=12`, `D<=4`), not in CI `all_passed` |
...
| [01-06 OMBU wavelet frames](01-geometry/06-ombu-wavelet-frames.md) | gated | `FrameSpec`; `sigma'` not admissible; not orthonormal / not compactly supported; G1–G3 CI; G4 denoising unearned, not in CI `all_passed` |
| [01-07 order as frequency](01-geometry/07-order-as-frequency-spectral-design.md) | gated | `BandPlan` / `peak_frequency`; pack order is a band selector, not Littlewood-Paley completeness; G1–G2/G4 earned; G3 unearned, not in CI `all_passed` |
| [01-08 tropical-log homotopy](01-geometry/08-tropical-log-homotopy.md) | gated | `omnibias.struct._core.tropical`; reuses `logsumexp_gap_bound`; G4 path-following **reported** (no driver wired to `anneal_descent`); cost vs `n`/`D` **reported** (refuses `n>10` or `D>3`), not in CI `all_passed` |
...
| [01-10 jet-bundle formalization](01-geometry/10-jet-bundle-formalization.md) | gated | Vocabulary / contact test, not a discovery and not a package; G1–G3 **earned** (`is_holonomic` 220/220, residual rates `~1/4` vs `~1/2`, dictionary terms in other specs) |
...
| [01-12 conjugate Hilbert tower](01-geometry/12-conjugate-hilbert-tower.md) | gated | Line Hilbert only; G1–G4 CI; G5 campaign-artifact **unearned** (matched-width ratio `0.978`, need `10x`), not in CI `all_passed` |
...
| Spec | Status | One line |
|---|---|---|
| [02-01 scan-net](02-architectures/01-scan-net-gridless-cnn.md) | gated | Stacked scan banks; equivariance per-layer, on-lattice, not `R^D`; G1/G2/G3/G5 earned (G3 wall/point vs named k-NN over two decades); G4 k-NN boundary **reported** (Scan-Net wins the constructive density fit; k-NN is allowed to win), not CI `all_passed` |
| [02-02 arrangement graph network](02-architectures/02-arrangement-graph-network.md) | gated | Sampled tope subgraph; temperature collapse; sound gap, not P vs NP; G3 vs k-NN **reported** (0-hop centroid; GNN / `RegionModels` stay `--full`); cost vs `n`/`D` **reported** (G1 tooling refuses `n>12` or `D>4`), not in CI `all_passed` |
| [02-03 jet-KAN](02-architectures/03-jet-kan-univariate-basis.md) | gated | Edge-wise univariate bases; exactness of the model jet, not the target; KA theorem does not justify; G1/G3/G5 CI; G2 cost **unearned** (~2.2x vs autodiff, need 5x), not CI `all_passed` |
...
| [02-05 multi-interface PINN](02-architectures/05-multi-interface-transmission-pinn.md) | gated | Parallel interfaces; `alpha -> inf` is sharpening, neither collapse; G3 vs PartitionedField / FBPINN **reported** (linear stand-in); G4 hard vs penalized **reported** (zero-coeff soft); training stays `--full`, not in CI `all_passed` |
| [02-06 potential theory and BEM-net](02-architectures/06-potential-theory-and-bem-net.md) | gated | PDE exact off-surface; BC approximated; linear constant-coeff homogeneous; G2 disc-accuracy **unearned** / `--full`; single-layer cost **reported**; G3 exterior win **reported** (pack-tree crossover absent; no volume PINN), not in CI `all_passed` |
...
| [02-08 equivariant and manifold scan](02-architectures/08-equivariant-and-manifold-scan.md) | gated | Gaussian-family steering only; discrete `C_L`, not SO(2)/SO(3); G5 anisotropic-interface **unearned** / `--full`; orbit cost **reported**, not in CI `all_passed` |
| [02-09 soliton tanh-method nets](02-architectures/09-soliton-tanh-method-networks.md) | gated | Tanh algebra, not a collapse; multi-kink is not the n-soliton formula; G4 PINN init-win **unearned** / `--full`; algebraic cost **reported**, not in CI `all_passed` |
| [02-10 Hermite ladder nets](02-architectures/10-hermite-ladder-oscillator-net.md) | gated | Raw tower is not the QHO eigenbasis; Rodrigues reweight required; G4 FermiNet many-body **unearned** / `--full`; exact-ladder vs FD **reported**; G5 anharmonic **reported** (loses to FD grid), not in CI `all_passed` |
...
| [02-14 Wilson-line holonomy band](02-architectures/14-wilson-line-holonomy-band.md) | gated | Closed form abelian + transverse-constant; open lines gauge-dependent; no YM / mass gap / continuum claim; G2 closed-form **earned** vs PRODUCT `substeps=4096`; G3 Magnus **reported**; G4 gauge covariance **reported** (no random-gauge API), not in CI `all_passed` |
...
| [03-12 exact jet line search](03-algorithms/12-exact-jet-line-search.md) | gated | Certified radius + `verify=True` never-worse; G1/G2/G3/G6 CI; G4 step-count **unearned** (strong Wolfe, 1.83x, need 2x), G5 order×depth crossover reported, not CI `all_passed` |
...
| Spec | Status | One line |
|---|---|---|
| [04-01 information geometry](04-bridges/01-information-geometry-exponential-family.md) | gated | Pack-parameter Fisher in `omnibias.curvature.information`; G1–G5 earned (`G_{delta,delta} ~ delta^2/720`) |
...
| [05-02 beyond-PDE applications](05-applications/02-beyond-pde-applications.md) | gated | Tabular arrangements (G1/G2/G3 earned; G4 reported, predictiveness `0.25`, not in CI `all_passed`); shape topology G6/G7 earned; sequence filter **earned** (G5 vs S4D; order-0 tail, width = horizon) |
...
| [06-03 packaging and rollout](06-program/03-packaging-and-rollout.md) | gated | G1–G5 **earned** (42 packages, 93/93 homes, Wave-0 A4–A7 recorded; G4 vacuous; G5 vacuous — 2 consumers, 389 lines, not promoted); no new distribution |
...
| [07-06 validated dynamics and orbits](07-frontier/06-validated-dynamics-and-orbits.md) | gated | Exact-Jacobian jet Lohner + width budget; G1–G6 CI; founding bias collapse, not temperature collapse; finite horizon, not a continuum existence theorem |
| [07-07 Nobel-adjacent domains](07-frontier/07-nobel-adjacent-domain-programs.md) | gated | Exact ladder + Harris layer + exact `dT/dθ`; G0–G6 CI; tooling, not a discovery; founding bias collapse, not temperature collapse |
...
| Spec | Status | One line |
|---|---|---|
| [08-01 training-idea ledger](08-training/01-training-idea-ledger.md) | gated | Taxonomy, rejects, recommended stack; trainers do not clear Hilbert stretch |
...
| [08-03 depth-causal local jet](08-training/03-depth-causal-local-jet.md) | gated | Forward `layer_jet` + local GN; `k` directions only; warm-start, not ImageNet; G1–G4 CI |
...
| [08-05 depth-causal residual](08-training/05-depth-causal-residual.md) | gated | March the PDE residual in network depth; not CCF Hilbert; G1–G3 CI |
...
| [08-08 implicit DEQ Newton](08-training/08-implicit-deq-newton.md) | gated | `u = sigma(W u + x)` with exact-`sigma'` IFT; no unrolled BPTT; G1–G3 CI |
...
is gated (G1–G5 earned). **01-02** is gated (G1–G4 CI-gated;
