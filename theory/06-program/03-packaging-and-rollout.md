...
- **Status**: shipped (G1–G5 **earned** on `theory_homes` smoke — 42 packages, allowlist empty, 125/125 specs name a home, Wave-0 A4–A7 recorded, no failed falsifier / no `retired` row; G4 vacuous; G5 vacuous — not promoted: 2 external consumers, 389 lines < 2000; no new distribution)
...
bit-identical, death reports a bound; G4 earned). Group 08 (tower-native trainers)
...
09-08…09-15, 09-20…09-23, 09-25…09-29) wait on a first-bet gate.
...
- **G4 deletion discipline.** Any spec whose falsifier failed is marked
  ...
  a one-line reason. Smoke-earned as a vacuous universal: no Wave-0 unit is
  **failed** and no index row is `retired`. Unearned leftover gates are not
...
- [x] `packages/omnibias-core/tests/test_theory_homes.py` with an empty
