...
- **Status**: gated (layer on 01-09; always `branch` / `condition` / `converged`; not a general PDE solver; G4 Burgers RH leftover-recorded / unearned, leftover #29 — clean speed from `affine_locus`, noisy contour stays `--full`, not in CI `all_passed`)
