...
- **Status**: gated (G1/G2/G4/G5 CI; G2 closed-form **earned** vs PRODUCT `substeps=4096`; G3 Magnus leftover-recorded / unearned, leftover #34 — no Magnus holonomy API; G4 gauge covariance **earned**, leftover #35 closed on tick #73 — `random_u1_gauge` + `g(hi) U g(lo)^{-1}`; abelian + transverse-constant closed form; no YM / mass gap / continuum claim)
