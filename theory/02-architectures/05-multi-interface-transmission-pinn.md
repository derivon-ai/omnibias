...
- **Status**: gated (`alpha -> inf` is sharpening, neither collapse; parallel interfaces only; G3 vs PartitionedField / FBPINN **reported** unearned — linear stand-in; G4 hard vs penalized **reported** unearned — zero-coeff soft; training stays `--full`)
