...
- **Status**: gated (1-D offsets; `eta=0` bit-identical; G3 complexity **earned**, leftover #13 closed on tick #74 — cached `O(p)` multipole `far_eval`, dense crossover)
...
- **G3 complexity.** Measured cost scales as predicted (near-linear in `N + M`)
  ...
  evaluation recorded.
  **Earned** (leftover #13 closed on tick #74) —
  `docs/benchmarks/pack_tree_smoke.json`. `far_eval` is an `O(p)`
  multipole when member orders match; `hierarchical_value` caches
  moments across `z`. Dense crossover on `M in {32, 320, 3200}`. In CI
  `all_passed`.
