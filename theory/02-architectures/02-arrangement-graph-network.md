...
- **Status**: gated (G1/G2 CI on `n<=12`; G3 vs k-NN **reported** / unearned — 0-hop centroid, GNN / `RegionModels` stay `--full`; sampled subgraph; temperature collapse; cost vs `n`/`D` **reported**, not in CI `all_passed`)
