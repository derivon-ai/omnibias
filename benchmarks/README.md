...
| `theory_homes.py` | `docs/benchmarks/theory_homes_smoke.json` | Gated 06-03: G1–G4 earned (93/93 homes; Wave-0 A4–A7 recorded; G4 vacuous); G5 not in `all_passed` |
...
| `equivariant_scan.py` | `docs/benchmarks/equivariant_scan_smoke.json` | Gated 02-08: gaussian steering; discrete `C_L`; G5 `--full`; orbit cost reported |
...
| `holonomy_band.py` | `docs/benchmarks/holonomy_band_smoke.json` | Gated 02-14: no YM / mass-gap claim; G2 earned; G3/G4 reported (no Magnus U / no random gauge) |
...
| `jetkan.py` | `docs/benchmarks/jetkan_smoke.json` | Wave-3 architecture 02-03: JetKAN G1/G3/G5; G2 cost unearned (~2.2x vs autodiff, need 5x); model-jet exactness, KA theorem does not justify |
...
| `multi_interface_pinn.py` | `docs/benchmarks/multi_interface_pinn_smoke.json` | Wave-3 architecture 02-05: sharpening, neither collapse; G3/G4 reported (stand-ins; training `--full`) |
| `jet_line_search.py` | `docs/benchmarks/jet_line_search_smoke.json` | Wave-3 algorithm 03-12: G1/G2/G3/G6 CI-gated; G4 unearned vs strong Wolfe; G5 order×depth crossover reported; not in CI `all_passed` |
