...
| Jet-bundle vocabulary (01-10 G1/G2; G3 not CI) | [`benchmarks/jet_bundle.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/jet_bundle.py) | dictionary, not a discovery; no `omnibias-jetbundle` package; G1/G2 **earned** (`is_holonomic` 220/220, residual rates `~1/4` vs `~1/2`); G3 vocabulary not in CI `all_passed`; smoke [`jet_bundle_smoke.json`](benchmarks/jet_bundle_smoke.json) |
