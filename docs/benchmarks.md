...
| Tropical homotopy (01-08 G1–G3; G4 `--full`) | [`benchmarks/tropical_homotopy.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/tropical_homotopy.py) | reuses `logsumexp_gap_bound`; temperature collapse; cost vs `n`/`D` reported with refuse `n>10`/`D>3`, **not** in CI `all_passed`; smoke [`tropical_homotopy_smoke.json`](benchmarks/tropical_homotopy_smoke.json) |
