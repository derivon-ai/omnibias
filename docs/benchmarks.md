...
| Jet-token transformer (09-02 G1–G3, **shipped**) | [`benchmarks/jet_token_transformer.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/jet_token_transformer.py) | `compose_jet` mix, not softmax-of-values; five-seed skill vs value-only on `sin x`; not ImageNet; founding bias collapse, not temperature collapse; smoke [`jet_token_transformer_smoke.json`](benchmarks/jet_token_transformer_smoke.json) |
