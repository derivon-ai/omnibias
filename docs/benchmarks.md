...
| Spectral design (01-07 G1/G2/G4; G3 reported) | [`benchmarks/spectral_design.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/spectral_design.py) | peak vs numerical argmax ≤ 1e-6; transfer magnitude within 2%; hole-detection; pack order is a band selector, not Littlewood-Paley completeness; G3 **reported** (`0/5` geometric and band-planned Mscale hits of the four-gap lstsq gate in 60 steps; `2x` ratio undefined), **not** in CI `all_passed`; smoke [`spectral_design_smoke.json`](benchmarks/spectral_design_smoke.json) |
...
| Jet-KAN (02-03 G1/G3/G5; G2 reported) | [`benchmarks/jetkan.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/jetkan.py) | model-jet vs inlined cubic B-spline; exactness is of the model jet, not the target; KA theorem does not justify; G2 order-6 jet vs autodiff at `L=3` **reported** (~2.2x, need 5x), **not** in CI `all_passed`; smoke [`jetkan_smoke.json`](benchmarks/jetkan_smoke.json) |
...
| OMBU frames (01-06 G1–G3; G4 reported) | [`benchmarks/ombu_frames.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/ombu_frames.py) | `sigma'` not admissible; G4 denoising **reported** (`5/5` MSE wins vs n=1, `0/5` skill vs identity, median skill `-1.32`; named G4 needs both), **not** in CI `all_passed`; smoke [`ombu_frames_smoke.json`](benchmarks/ombu_frames_smoke.json) |
...
| Conjugate Hilbert (01-12 G1–G4; G5 reported) | [`benchmarks/conjugate_hilbert.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/conjugate_hilbert.py) | line Hilbert only; G1–G4 in CI `all_passed`; G5 **reported** on the CCF profile-fitting smoke (matched-width residual ratio `0.978`, need `10x`); not a stretch-gate clearing; **not** in CI `all_passed`; smoke [`conjugate_hilbert_smoke.json`](benchmarks/conjugate_hilbert_smoke.json) |
