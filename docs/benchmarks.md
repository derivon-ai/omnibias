...
| Linearizing transforms (02-13, **shipped**) | [`benchmarks/linearizing_transforms.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/linearizing_transforms.py) | named maps only; G1 jet identity CI; G6 torch/jax parity CI; G2 n-soliton **leftover-recorded** (leftover #51); G3 Burgers **leftover-recorded** (leftover #39; no train vs direct PINN), **not** in CI `all_passed`; 03-11 search not claimed; smoke [`linearizing_transforms_smoke.json`](benchmarks/linearizing_transforms_smoke.json) |
