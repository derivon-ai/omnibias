...
| Holonomy band (02-14) | [`benchmarks/holonomy_band.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/holonomy_band.py) | abelian + transverse-constant closed form; no YM / mass gap; G2 closed-form **earned** vs PRODUCT `substeps=4096`; G3 Magnus **leftover-recorded** (leftover #34; no Magnus holonomy API); G4 gauge covariance **reported** (no random-gauge API), **not** in CI `all_passed`; smoke [`holonomy_band_smoke.json`](benchmarks/holonomy_band_smoke.json) |
