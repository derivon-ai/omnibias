...
| Multi-interface transmission PINN (02-05) | [`benchmarks/multi_interface_pinn.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/multi_interface_pinn.py) | sharp-limit piecewise poly; residual ~`1/alpha`; `alpha -> inf` is interface sharpening, neither collapse; G3 vs PartitionedField / FBPINN **leftover-recorded** (leftover #37; linear stand-in); G4 hard vs penalized **reported** (zero-coeff soft); training stays `--full`, **not** in CI `all_passed`; smoke [`multi_interface_pinn_smoke.json`](benchmarks/multi_interface_pinn_smoke.json) |
