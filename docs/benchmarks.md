...
| Face-Net (02-02) | [`benchmarks/arrangement_graph.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/arrangement_graph.py) | sampled tope subgraph; temperature collapse; G3 vs k-NN **leftover-recorded** (leftover #28; 0-hop centroid; GNN / `RegionModels` stay `--full`); cost vs `n`/`D` reported with cutoff `n<=12`, `D<=4`, **not** in CI `all_passed`; smoke [`arrangement_graph_smoke.json`](benchmarks/arrangement_graph_smoke.json) |
