...
| Lie symmetry discovery (03-11 G1–G6, **shipped**) | [`benchmarks/symmetry_discovery.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/symmetry_discovery.py) | in-ansatz heat/wave/Burgers/KdV/Laplace/Fisher/Boussinesq recovery; separation `> 1e6`; FD prolongation gets the heat rank wrong; two-decade threshold stability; wave Noether residual `<= 1e-10`; negative control is trivial; founding bias collapse, not temperature collapse; point symmetries only; smoke [`symmetry_discovery_smoke.json`](benchmarks/symmetry_discovery_smoke.json) |
