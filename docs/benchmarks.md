...
| Public CSV lynx–hare discovery | [`examples/symbolic_discovery/public_csv_discovery`](https://github.com/derivon-ai/omnibias/tree/main/examples/symbolic_discovery/public_csv_discovery) | train-only cubic-spline STLSQ + RK4 rollout vs persist / linear / FD on Hudson Bay pelts; recovers LV `xy` signs on the synthetic orbit and the public table; RF jet reported (not the STLSQ interpolant); not a new law; smoke [`public_csv_discovery_smoke.json`](benchmarks/public_csv_discovery_smoke.json) |
