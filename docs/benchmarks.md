...
| Sequence transverse vs S4D (05-02 G5 / Wave-0 A5) | [`benchmarks/sequence_transverse.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/sequence_transverse.py) | Designed causal `sigma^(n)` FIR vs named S4D (Gu et al. 2022, N=1) on an AR(1) leaky integrator; matched 4 parameters; 5 seeds, worst-seed `R^2` gap; **failed** (worst gap `0.652`, need `<= 0.02`); sequence submodule retired; smoke [`sequence_transverse_smoke.json`](benchmarks/sequence_transverse_smoke.json) |
...
| Conjugate Hilbert (01-12 G1–G4; G5 not CI) | [`benchmarks/conjugate_hilbert.py`](https://github.com/derivon-ai/omnibias/blob/main/benchmarks/conjugate_hilbert.py) | line Hilbert only; G5 is a projection defect, not a stretch-gate clearing; smoke [`conjugate_hilbert_smoke.json`](benchmarks/conjugate_hilbert_smoke.json) |
