...
are **reported**. G5 anharmonic honesty is **reported**: the oscillator
ground Rayleigh loses to a Dirichlet FD grid on `V = x^2/2 + x^4`; the
previous untimed `passed=True` stub is withdrawn. Cost and G5 are not
