...
are CI-gated. G4 Burgers Rankine–Hugoniot is **reported** unearned:
`affine_locus` recovers the published-unit speed, but the noisy-data
skill versus contour extraction stays `--full` (units are not fit from
samples). The previous smoke-geometry `passed=True` stub is withdrawn.
G4 is not in CI `all_passed`. Status is **gated**, not shipped. See
