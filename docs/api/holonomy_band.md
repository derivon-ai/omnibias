...
`||A|| L >= pi`, but no Magnus-truncated holonomy is wired. G4 gauge
covariance is **reported** unearned: the open-line flag and a
forward-back `band_wilson_loop` identity are measured, but a random
`g(x_hi) U g(x_lo)^{-1}` path is not wired. Previous open-line-flag
stub withdrawn from named G4. G3 and G4 are not in CI `all_passed`.
