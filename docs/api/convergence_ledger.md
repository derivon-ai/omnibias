...
`external_premises` is empty. `navier_stokes_proof_claim` additionally
requires `parent == NS_PARENT` (Clay A/B). A discharged C/D ledger
does not earn it. The three curated ledgers (NS exponent,
