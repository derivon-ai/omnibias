...
| [`pinn_causal_marching.py`](examples/pinn_causal_marching.py) | PyTorch | Causal time-marching CI smoke via `omnibias.pinn.train.torch.march_solve` (`ic_fn`); acceptance is `benchmarks/causal_marching.py` (heat + reaction); recipe [`cookbook/pinn-causal-marching.md`](cookbook/pinn-causal-marching.md) |
