...
| Jacobian conjecture (`n>=3` already false) | Exact `Q` Alpöge replay + blind deg-2 / deg-3 tangent sweep via `run_discovery` in `omnibias.holonomic.keller`. `n=2` is the finite universal `C_box` plus the Gabber inverse-degree test in `omnibias.holonomic.jacobian_n2` (identical Jac + grid collision or inverse-jet failure). A miss is not the parent |
...
| omnibias refuted Keller / the Jacobian conjecture | `omnibias.holonomic.keller`; engine found a map **in the sweep family**. An `n=2` claim is only `escalate_n2_result` on an exact violator (grid collision or Gabber fail) |
