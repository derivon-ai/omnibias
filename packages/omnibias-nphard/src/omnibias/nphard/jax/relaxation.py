...
r"""Differentiable NP-hard relaxation layer (JAX): a thin wrapper over the QUBO relaxation.

Bit-identical twin of :mod:`omnibias.nphard.torch.relaxation` (float64). :func:`relax`
reduces an NP-hard family problem to its QUBO (``problem.to_qubo()``) and hands it to
:func:`omnibias.qubo.jax.qubo_relaxation`, which descends a soft assignment
...
weights, use :func:`omnibias.nphard.jax.qap_decision_cost` (backprop through the solver).
...
derivative ``sigma^(K-1)``; see :mod:`omnibias.jax` and ``docs/theory.md``).
"""
...
def relax(problem: Any, schedule: AnnealSchedule | None = None) -> Array:
    ...
    """
    result: Array = qubo_relaxation(problem.to_qubo(), schedule=schedule)
    return result
