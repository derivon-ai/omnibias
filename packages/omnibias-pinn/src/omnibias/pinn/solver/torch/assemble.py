...
def _bc_rows(
    ...
    if bc.kind == "periodic":
        # Periodicity is carried by the (spectral) ansatz / domain, not a row.
        return field.W.weight.new_zeros(0)
    rows: list[Tensor] = []
...
def build_plan(
    ...
    return CollocationPlan(interior_state, tuple(system.residuals), bc_terms, ic_terms)
