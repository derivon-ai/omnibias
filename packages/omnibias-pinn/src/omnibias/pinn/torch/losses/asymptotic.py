...
"""Asymptotic / removable boundary-condition losses (torch twin).

These helpers turn the differentiable jet-``lim`` operator into trainable PINN
losses. They are built on :func:`omnibias.torch.mlp_jet` (the *exact* directional
Taylor jet of a deep MLP along a ray ``x(t) = x0 + t v``) and
:func:`omnibias.torch.lhopital_ratio` (the differentiable L'Hopital limit), so a
...
and TorchScript / ``vmap`` friendly. The exponent / target may itself be learnable.

Bit-identical twin of :mod:`omnibias.pinn.jax.losses.asymptotic`.
"""
...
import torch
from omnibias.torch.jet import lhopital_ratio, mlp_jet
from torch import Tensor

if TYPE_CHECKING:  # pragma: no cover -- typing-only
    from omnibias.core.spec import ActivationSpec

    TorchLayer = tuple[Tensor, Tensor | None, ActivationSpec[Tensor] | str | None]


def _power_jet(rate: int, order: int, like: Tensor) -> Tensor:
    ...
    """
    coeffs = torch.zeros(order + 1, dtype=like.dtype, device=like.device)
    coeffs[rate] = 1.0
    return coeffs
...
def network_ray_jet(
    ...
    *,
    ...
    Thin wrapper over :func:`omnibias.torch.mlp_jet` that selects output channel
    ``out_index``; returns a tensor of shape ``(order + 1,)`` whose ``k``-th
...
def far_field_decay_loss(
    ...
    *,
    ...
    return float(weight) * torch.mean(net_jet * net_jet)
