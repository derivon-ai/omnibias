...
"""02-14 G4 gauge covariance is earned and in CI all_passed."""
...
def test_g4_gauge_covariance_is_earned_and_in_all_passed() -> None:
    ...
    assert int(g4["leftover_tick"]) == 73
    assert g4["in_ci_all_passed"] is True
    assert g4["open_line_flagged"] is True
    assert g4["open_holonomy_not_invariant"] is True
    assert g4["random_gauge_api"] is True
    assert g4["random_gauge_within_4_ulp"] is True
    assert float(g4["random_gauge_max_ulps"]) <= float(g4["loop_ulp_max"])
    assert g4["stays_full"] is False
    ...
    assert payload["honesty"]["g4_random_gauge_api"] is True
