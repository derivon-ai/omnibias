...
"""01-08 cost vs n/D stays leftover-recorded; G4 path-following is earned."""
...
def test_cost_vs_n_d_is_reported_and_out_of_all_passed() -> None:
    ...
    assert int(g4["leftover_tick"]) == 72
    assert g4["in_ci_all_passed"] is True
    assert g4["path_follow_api"] is True
    assert g4["anneal_descent_wired"] is True
    assert g4["relaxed_hess_exported"] is True
    assert g4["stays_full"] is False
    assert int(g4["wins"]) == 5
    ...
    assert payload["honesty"]["g4_path_follow_api"] is True
