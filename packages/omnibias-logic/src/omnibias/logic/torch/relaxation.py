...
r"""Differentiable annealed #SAT model-finder relaxation (torch).

Bit-identical twin of :mod:`omnibias.logic.jax.relaxation` (float64). For #SAT every clause
...
:func:`omnibias.discrete.torch.anneal_descent`, which anneals ``x = sigmoid(beta theta)`` onto
a binary vertex as ``beta -> inf`` (unrolled for backprop). Decoding the soft output yields a
model iff its energy is ``0``; distinct decoded models are sound witness lower bounds for
...
def sat_relaxation(problem: ModelCountProblem, schedule: AnnealSchedule | None = None) -> Tensor:
    ...
    """
    result: Tensor = maxsat_relaxation(problem.as_maxsat, schedule)
    return result
