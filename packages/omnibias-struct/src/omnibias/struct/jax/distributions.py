...
r"""Distribution operators over the Gibbs path distribution ``p_beta`` (JAX).

Bit-identical twin of :mod:`omnibias.struct.torch.distributions` for the differentiable ops.
...
def path_entropy(graph: Hypergraph, edge_weights: Array, beta: float = 1.0) -> Array:
    ...
    entropy: Array = beta * (value - expected_score)
    return entropy
...
def sample_paths(
    ...
    *,
    ...
    Returns ``(counts, samples)``: ``counts`` is a ``(num_samples, num_edges)`` array of
    ...
    """
    weights = [float(x) for x in np.asarray(edge_weights).reshape(-1)]
    counts, samples = sample_derivations(graph, weights, beta, num_samples, seed=seed)
    return jnp.asarray(counts, dtype=edge_weights.dtype), samples
...
def gumbel_relaxed_sample(
    ...
    *,
    ...
    u = rng.random(size=int(edge_weights.shape[0]))
    gumbel = jnp.asarray(-np.log(-np.log(u)), dtype=edge_weights.dtype)
    perturbed = edge_weights + gumbel
    relaxed: Array = semiring_marginals(graph, perturbed, beta)
    if not hard:
        return relaxed
    _score, deriv = best_derivation(graph, [float(x) for x in np.asarray(perturbed).reshape(-1)])
    hard_ind = jnp.zeros_like(edge_weights)
    for e in deriv:
        hard_ind = hard_ind.at[e].add(1.0)
    straight_through: Array = hard_ind + (relaxed - lax.stop_gradient(relaxed))
    return straight_through


def topk_paths(
    graph: Hypergraph, edge_weights: Array, k: int
...
def topk_free_energy(graph: Hypergraph, edge_weights: Array, k: int, beta: float = 1.0) -> Array:
    ...
    scores = jnp.stack(
        [
            jnp.stack([edge_weights[e] for e in deriv]).sum() if deriv else jnp.zeros((), dtype=edge_weights.dtype)
            for _score, deriv in kbest
        ]
    )
    free_energy: Array = logsumexp_beta(scores, beta, axis=-1)
    return free_energy
