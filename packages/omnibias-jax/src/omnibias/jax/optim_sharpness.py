...
r"""Sharpness-scheduled cubic / learning-rate step (theory 08-06), JAX twin.

Exact Hessian-vector products (:func:`omnibias.jax.optim.hvp`) and
...
when matching the torch twin to ``1e-10``.
"""
...
def _math_isfinite(value: float) -> bool:
...
def sharpness_lambda_max(
    ...
    *,
    ...
    from omnibias.jax.optim import hvp, lanczos_tridiag

    if int(n_lanczos) < 1:
        raise ValueError(f"n_lanczos must be >= 1, got {n_lanczos}")
    p = jnp.reshape(params, (-1,))
    start = jnp.reshape(probe, (-1,)) if probe is not None else jax.grad(loss_fn)(p)
    if float(jnp.linalg.norm(start)) == 0.0:
        start = jnp.zeros_like(p).at[0].set(1.0)

    def matvec(v: Array) -> Array:
        return hvp(loss_fn, p, v)

    _q, tri = lanczos_tridiag(matvec, start, int(n_lanczos))
    return float(jnp.max(jnp.linalg.eigvalsh(tri)))
...
def sharpness_scheduled_step(
    ...
    *,
    ...
    if not _math_isfinite(report.scheduled) or report.scheduled <= 0.0:
        raise ValueError(
    ...
    from omnibias.jax.optim import cubic_regularized_newton_step
...
def sharpness_scheduled_minimize(
    ...
    *,
    ...
    for _ in range(int(steps)):
        ...
        if not _math_isfinite(losses[-1]) or not bool(jnp.all(jnp.isfinite(p))):
            break
