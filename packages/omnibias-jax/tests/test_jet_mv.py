...
"""Validation suite for the jax *multivariate* Faà di Bruno jet kernel.

Oracles (float64):

* full mixed-partial tensor of a deep MLP vs nested ``jax.jacfwd`` (every
  multi-index up to total order ``N``),
* gradient / Hessian extraction vs ``jax.grad`` / ``jax.hessian``,
* directional restriction: contracting the multivariate jet along ``v`` must
  reproduce the (independently validated) directional :func:`mlp_jet`,
* affine linearity per coefficient, and the order-cap / shape error paths.
"""
...
def _full_deriv_tensor(f, x0, n: int):
    """The order-``n`` derivative tensor ``D^n f(x0)`` via nested jacfwd."""
    fn = f
    for _ in range(n):
        fn = jax.jacfwd(fn)
    return fn(x0)
...
def test_full_multivariate_capstone() -> None:
    # Deeper network, higher total order, two output components: assert *every*
    # mixed partial up to order 5 in 3 variables matches nested AD.
...
def test_gradient_and_hessian_match_autodiff() -> None:
    ...
    assert jnp.allclose(g, jax.grad(f)(x0), rtol=1e-10, atol=1e-10)
    assert jnp.allclose(h, jax.hessian(f)(x0), rtol=1e-10, atol=1e-10)
    # Hessian is symmetric.
    assert jnp.allclose(h, h.T, rtol=0, atol=1e-12)
...
def test_directional_restriction_matches_mlp_jet() -> None:
    ...
    # d^k/dt^k f(x0 + t v) = sum_{|alpha|=k} (k!/alpha!) D^alpha f v^alpha.
    idx = multi_indices(dim, order)
    v_np = np.asarray(v)
    by_degree: dict[int, jnp.ndarray] = {}
    for alpha in idx:
        k = sum(alpha)
        afact = 1
        valpha = 1.0
        ...
        by_degree[k] = by_degree.get(k, jnp.zeros_like(contrib)) + contrib

    directional = jet_to_tower(mlp_jet(x0, v, layers, order))  # (order+1, 1)
    for k in range(order + 1):
        assert jnp.allclose(by_degree[k], directional[k], rtol=1e-9, atol=1e-10)
...
def test_affine_jet_mv_linear_per_coefficient() -> None:
    ...
    z_jet = jnp.asarray(rng.normal(size=(m, 5)))
...
def test_identity_jet_seed() -> None:
    x0 = jnp.asarray([0.3, -1.1, 2.0])
    order = 2
    jet = identity_jet(x0, order)
    idx = multi_indices(3, order)
    # value row reproduces x0; unit rows are basis vectors; the rest vanish.
    assert jnp.allclose(jet[0], x0)
    for i, alpha in enumerate(idx):
        if sum(alpha) == 0:
            assert jnp.allclose(jet[i], x0)
        elif sum(alpha) == 1:
            j = alpha.index(1)
            expected = jnp.asarray([1.0 if k == j else 0.0 for k in range(3)])
            assert jnp.allclose(jet[i], expected)
        else:
            assert jnp.allclose(jet[i], jnp.zeros(3))
...
def test_order_cap_raises_value_error() -> None:
    ...
    # arctan caps at order 2; an order-3 mv jet must fail.
