...
"""Exact Hyvärinen score matching (torch; theory 09-21).
...
def exact_div_neg_id(xs: Tensor | float | int = 1) -> float:
    if isinstance(xs, Tensor):
        dim = int(xs.reshape(-1).numel()) if xs.ndim > 0 and int(xs.numel()) > 1 else 1
...
def exact_score_matching_loss(
    ...
    *,
    ...
    if isinstance(xs, Tensor):
        vals = [float(v) for v in xs.reshape(-1).detach().tolist()]
    else:
        vals = [float(xs)]
    if cfg.kind == "dsm":
