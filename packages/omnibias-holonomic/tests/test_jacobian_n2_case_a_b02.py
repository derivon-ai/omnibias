...
"""Case A (b02, b03, b04) leftover over Q. Proof-claim stays False."""
...
def test_statement_is_universal_and_parent_stays_open() -> None:
    ...
    assert statement.name == CASE_A_B02_KIND
    assert "only at the origin" in statement.obligation
    assert "Case A chart is then empty" in statement.obligation
    assert "not jacobian_conjecture_n2" in statement.obligation
    assert "Case B is a later slice" in statement.obligation


def test_extract_replays_named_generators() -> None:
    assert case_a_b02_generators() == named_case_a_b02_generators()
...
def test_float_witness_is_blocked() -> None:
    verdict = seal_case_a_b02({"witness": [1.0, 0, 0]})
...
def test_replay_and_forged_proof_claim() -> None:
    ...
    assert replay_case_a_b02(sealed.certificate) is True
    forged = {
        **sealed.certificate,
        "honesty": {
...
def test_catalog_and_machine() -> None:
    ...
    verdict = machine.evaluate(
        Conjecture(name="case-a-b02", kind=JACOBIAN_N2_CASE_A_B02, data={}),
        replay=True,
    )
    assert verdict.proved
