...
"""JAX chemistry / transport ops (mirrors :mod:`omnibias.fields.torch.ops.chemistry`).
...
the Phase-1 gradient / diffusion primitives, so the torch twin is bit-identical
...
from omnibias.fields.jax.ops.basic import derivative, gradient, laplacian, value
from omnibias.fields.jax.ops.conservation import (
    diffusive_flux,
...
def nernst_planck_residual(
    ...
    *,
    ...
    migration = (valence * mobility * faraday) * (
        (grad_c * grad_phi).sum(axis=-1) + c * lap_phi
    )
...
def reaction_diffusion_residual(
    ...
    *,
    scalar: str,
    diffusivity: float | str = 1.0,
    reaction: Callable[[Array], Array] | str | float | None = None,
    source: str | float | None = None,
) -> Array:
    r""":math:`\partial_t c - \nabla\cdot(D\nabla c) - R(c) - s`.
