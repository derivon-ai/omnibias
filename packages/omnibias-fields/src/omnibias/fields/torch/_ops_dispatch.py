...
"""Torch ops dispatch: thin entry points the views forward into.

Each function here takes a :class:`FieldState` plus the args specific to
the op and dispatches to the field-type-specific kernel inside
``omnibias.fields.torch.ops.*``. The dispatch lives at the package root
(not behind a ``ops/`` subpackage import) so that the views' delegation
``state.ops.<fn>(...)`` is a single-step lookup with no circular imports.

The functions below are all wrappers: the actual math lives in
``omnibias.fields.torch.ops.{basic,high_order,nonlinear,vector}``. This
module is the thin router.
"""
...
def value(state: FieldState, name: str) -> Tensor:
    from omnibias.fields.torch.ops.basic import value as _value
...
def derivative(
    ...
    from omnibias.fields.torch.ops.basic import derivative as _derivative
...
def gradient(
    ...
    from omnibias.fields.torch.ops.basic import gradient as _gradient
    return _gradient(state, name, axes=axes)


def divergence(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.basic import divergence as _divergence
...
def laplacian(
    ...
    from omnibias.fields.torch.ops.basic import laplacian as _laplacian
    return _laplacian(state, name, axes=axes)


def stack_components(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.basic import stack_components as _stack
...
def vector_derivative(
    ...
    from omnibias.fields.torch.ops.basic import vector_derivative as _vd
...
def mixed_partial(
    ...
    from omnibias.fields.torch.ops.basic import mixed_partial as _mp
    return _mp(state, name, axes, orders)


# ---------------- high-order ops (high_order.py) ---------------


def biharmonic(state: FieldState, name: str) -> Tensor:
    from omnibias.fields.torch.ops.high_order import biharmonic as _b
    return _b(state, name)


def polylaplacian(state: FieldState, name: str, *, k: int) -> Tensor:
    from omnibias.fields.torch.ops.high_order import polylaplacian as _pl
...
def hessian(
    ...
    *,
    ...
    from omnibias.fields.torch.ops.high_order import hessian as _h
    return _h(state, name, axes=axes)


def spatial_hessian(state: FieldState, name: str) -> Tensor:
    from omnibias.fields.torch.ops.high_order import spatial_hessian as _sh
...
def gradient_of_derivative(
    ...
    from omnibias.fields.torch.ops.high_order import gradient_of_derivative as _god
    return _god(state, name, axis=axis)


def jacobian(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.high_order import jacobian as _j
...
def vector_hessian(
    ...
    *,
    ...
    from omnibias.fields.torch.ops.high_order import vector_hessian as _vh
    return _vh(state, names, axes=axes)


def vector_laplacian(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.high_order import vector_laplacian as _vl
    return _vl(state, names)


def vector_biharmonic(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.high_order import vector_biharmonic as _vb
    return _vb(state, names)


def vector_polylaplacian(state: FieldState, names: tuple[str, ...], *, k: int) -> Tensor:
    from omnibias.fields.torch.ops.high_order import vector_polylaplacian as _vp
    return _vp(state, names, k=k)


# ---------------- nonlinear ops (nonlinear.py) -----------------
...
def advection(
    ...
    *,
    ...
    from omnibias.fields.torch.ops.nonlinear import advection as _a
...
def material_derivative(
    ...
    *,
    ...
    from omnibias.fields.torch.ops.nonlinear import material_derivative as _md
...
def p_laplacian(
    ...
    from omnibias.fields.torch.ops.nonlinear import p_laplacian as _pl
...
def directional_derivative(
    ...
    from omnibias.fields.torch.ops.nonlinear import directional_derivative as _dd
    return _dd(state, name, direction=direction)


# ---------------- vector ops (vector.py) -----------------------


def curl(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.vector import curl as _c
...
def strain_rate(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.vector import strain_rate as _s
    return _s(state, names)


def deformation_gradient(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.vector import deformation_gradient as _d
    return _d(state, names)


def spatial_jacobian(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.vector import spatial_jacobian as _sj
    return _sj(state, names)


# ---------------- integration / norms (integral.py, norms.py) --


def integrate(state: FieldState, name: str, *, rule: Any) -> Tensor:
    from omnibias.fields.torch.ops.integral import integrate as _i
...
def line_integral(
    ...
    from omnibias.fields.torch.ops.integral import line_integral as _li
...
def inner_product(
    ...
    from omnibias.fields.torch.ops.norms import inner_product as _ip
    return _ip(state, name_a, name_b, rule=rule, weight=weight)


def l2_norm(state: FieldState, name: str, *, rule: Any) -> Tensor:
    from omnibias.fields.torch.ops.norms import l2_norm as _l2
...
def sobolev_norm(
    ...
    from omnibias.fields.torch.ops.norms import sobolev_norm as _sn
    return _sn(state, name, rule=rule, k=k, weights=weights)


# ---------------- tensor divergence (tensor.py) ----------------


def tensor_divergence(state: FieldState, sigma_names: Any) -> Tensor:
    from omnibias.fields.torch.ops.tensor import tensor_divergence as _td
    return _td(state, sigma_names)


# ---------------- complex / Wirtinger (complex.py) -------------
...
def dz(
    ...
    from omnibias.fields.torch.ops.complex import dz as _dz
...
def dzbar(
    ...
    from omnibias.fields.torch.ops.complex import dzbar as _dzbar
...
def gradient_of_divergence(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.vector import gradient_of_divergence as _gd
    return _gd(state, names)


def curl_of_curl(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.vector import curl_of_curl as _cc
    return _cc(state, names)


def rate_of_rotation_tensor(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.vector import rate_of_rotation_tensor as _rr
    return _rr(state, names)


def div(state: FieldState, names: tuple[str, ...]) -> Tensor:
    """Alias for :func:`divergence`."""
    return divergence(state, names)


def rot(state: FieldState, names: tuple[str, ...]) -> Tensor:
    """Alias for :func:`curl`."""
    return curl(state, names)
...
def grad_squared_norm(state: FieldState, name: str) -> Tensor:
    from omnibias.fields.torch.ops.conservation import grad_squared_norm as _g
    return _g(state, name)


def gradient_of_composition(state: FieldState, name: str, fprime: Any) -> Tensor:
    from omnibias.fields.torch.ops.conservation import gradient_of_composition as _gc
...
def laplacian_of_composition(
    ...
    from omnibias.fields.torch.ops.conservation import laplacian_of_composition as _lc
...
def diffusive_flux(
    ...
    from omnibias.fields.torch.ops.conservation import diffusive_flux as _df
    return _df(state, name, diffusivity=diffusivity)


def flux_divergence(state: FieldState, names: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.conservation import flux_divergence as _fd
...
def variable_coefficient_diffusion(
    ...
    from omnibias.fields.torch.ops.conservation import variable_coefficient_diffusion as _vd
...
def conservation_residual(
    ...
    from omnibias.fields.torch.ops.conservation import conservation_residual as _cr
...
def advection_diffusion_residual(
    ...
    from omnibias.fields.torch.ops.conservation import advection_diffusion_residual as _ad
    return _ad(
...
def dalembertian(
    ...
    from omnibias.fields.torch.ops.conservation import dalembertian as _db
...
def wave_operator(
    ...
    from omnibias.fields.torch.ops.conservation import wave_operator as _wo
...
def skew_symmetric_advection(
    ...
    from omnibias.fields.torch.ops.nonlinear import skew_symmetric_advection as _ssa
    return _ssa(state, velocity=velocity, scalar=scalar)


def tensor_double_dot(a: Tensor, b: Tensor) -> Tensor:
    from omnibias.fields.torch.ops.tensor import tensor_double_dot as _tdd
    return _tdd(a, b)
...
def velocity_from_streamfunction(state: FieldState, psi: str) -> Tensor:
    from omnibias.fields.torch.ops.mechanics import velocity_from_streamfunction as _v
    return _v(state, psi)


def vorticity_from_streamfunction(state: FieldState, psi: str) -> Tensor:
    from omnibias.fields.torch.ops.mechanics import vorticity_from_streamfunction as _w
    return _w(state, psi)
...
def newtonian_stress(
    ...
    from omnibias.fields.torch.ops.mechanics import newtonian_stress as _ns
...
def linear_elastic_stress(
    ...
    from omnibias.fields.torch.ops.mechanics import linear_elastic_stress as _le
...
def viscous_dissipation(
    ...
    from omnibias.fields.torch.ops.mechanics import viscous_dissipation as _vd
    return _vd(state, velocity, viscosity=viscosity)


def stress_divergence(state: FieldState, sigma_names: Any) -> Tensor:
    from omnibias.fields.torch.ops.mechanics import stress_divergence as _sd
...
def stokes_residual(
    ...
    from omnibias.fields.torch.ops.mechanics import stokes_residual as _sr
    return _sr(
...
def navier_cauchy_residual(
    ...
    from omnibias.fields.torch.ops.mechanics import navier_cauchy_residual as _nc
    return _nc(
...
def fickian_flux(state: FieldState, name: str, *, diffusivity: float | str = 1.0) -> Tensor:
    from omnibias.fields.torch.ops.chemistry import fickian_flux as _ff
...
def nernst_planck_flux(
    ...
    from omnibias.fields.torch.ops.chemistry import nernst_planck_flux as _np
    return _np(
...
def darcy_flux(
    ...
    from omnibias.fields.torch.ops.chemistry import darcy_flux as _df
...
def nernst_planck_residual(
    ...
    from omnibias.fields.torch.ops.chemistry import nernst_planck_residual as _npr
    return _npr(
...
def reaction_diffusion_residual(
    ...
    from omnibias.fields.torch.ops.chemistry import reaction_diffusion_residual as _rd
    return _rd(
...
def poisson_residual(
    ...
    from omnibias.fields.torch.ops.chemistry import poisson_residual as _pr
...
def faraday_residual(
    ...
    from omnibias.fields.torch.ops.electromagnetism import faraday_residual as _f
...
def ampere_residual(
    ...
    from omnibias.fields.torch.ops.electromagnetism import ampere_residual as _a
...
def gauss_residual(
    ...
    from omnibias.fields.torch.ops.electromagnetism import gauss_residual as _g
    return _g(state, electric=electric, charge=charge)


def gauss_magnetic_residual(state: FieldState, *, magnetic: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.electromagnetism import gauss_magnetic_residual as _gm
...
def poynting_vector(
    ...
    from omnibias.fields.torch.ops.electromagnetism import poynting_vector as _p
    return _p(state, electric=electric, magnetic=magnetic)


def magnetic_field_from_potential(state: FieldState, *, potential: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.electromagnetism import magnetic_field_from_potential as _m
...
def electric_field_from_potentials(
    ...
    from omnibias.fields.torch.ops.electromagnetism import electric_field_from_potentials as _e
...
def lorenz_gauge_residual(
    ...
    from omnibias.fields.torch.ops.electromagnetism import lorenz_gauge_residual as _l
...
def vector_dalembertian(
    ...
    from omnibias.fields.torch.ops.electromagnetism import vector_dalembertian as _vd
...
def current_density(state: FieldState, *, magnetic: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.mhd import current_density as _cd
...
def lorentz_force(
    ...
    from omnibias.fields.torch.ops.mhd import lorentz_force as _lf
    return _lf(state, magnetic=magnetic, current=current)


def magnetic_pressure(state: FieldState, *, magnetic: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.mhd import magnetic_pressure as _mp
...
def maxwell_stress_tensor(
    ...
    from omnibias.fields.torch.ops.mhd import maxwell_stress_tensor as _mst
    return _mst(state, magnetic=magnetic, electric=electric)


def magnetic_divergence(state: FieldState, *, magnetic: tuple[str, ...]) -> Tensor:
    from omnibias.fields.torch.ops.mhd import magnetic_divergence as _md
...
def induction_residual(
    ...
    from omnibias.fields.torch.ops.mhd import induction_residual as _ir
...
def ideal_mhd_momentum_residual(
    ...
    from omnibias.fields.torch.ops.mhd import ideal_mhd_momentum_residual as _mm
    return _mm(
...
def vlasov_residual(
    ...
    force: tuple[str, ...] | Tensor | None = None, mass: float = 1.0,
) -> Tensor:
    from omnibias.fields.torch.ops.kinetic import vlasov_residual as _vr
    return _vr(
...
def bgk_collision(
    state: FieldState, name: str, *, equilibrium: str | Tensor, tau: float,
) -> Tensor:
    from omnibias.fields.torch.ops.kinetic import bgk_collision as _bc
...
def bgk_vlasov_residual(
    state: FieldState, name: str, *,
    position_axes: tuple[str, ...], velocity_axes: tuple[str, ...],
    equilibrium: str | Tensor, tau: float,
    force: tuple[str, ...] | Tensor | None = None, mass: float = 1.0,
) -> Tensor:
    from omnibias.fields.torch.ops.kinetic import bgk_vlasov_residual as _bvr
    return _bvr(
        state, name, position_axes=position_axes, velocity_axes=velocity_axes,
...
def maxwellian(
    ...
    temperature: float | Tensor = 1.0, mass: float = 1.0,
) -> Tensor:
    from omnibias.fields.torch.ops.kinetic import maxwellian as _mx
    return _mx(
...
def number_density(state: FieldState, name: str, *, rule: Any) -> Tensor:
    from omnibias.fields.torch.ops.kinetic import number_density as _nd
...
def momentum_density(
    ...
    from omnibias.fields.torch.ops.kinetic import momentum_density as _md
...
def kinetic_energy_density(
    ...
    from omnibias.fields.torch.ops.kinetic import kinetic_energy_density as _ked
    return _ked(state, name, rule=rule, velocity_axes=velocity_axes)


# ---------------- registry helper for views ---------------------


def list_ops() -> tuple[str, ...]:
    """Names of all dispatchable ops on this backend."""
    return tuple(sorted(
