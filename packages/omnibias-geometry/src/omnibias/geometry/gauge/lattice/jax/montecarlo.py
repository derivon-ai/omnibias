...
"""SU(2) lattice Monte Carlo driver for plaquette and glueball correlator evidence (JAX)."""
...
from omnibias.geometry.gauge.lattice.jax.observables import (
    average_plaquette,
...
from omnibias.geometry.gauge.lattice.jax.su2 import random_links, sweep
...
def run_lattice_mc(
    *,
    ...
    links = random_links(shape, init_key, dtype=jnp.float64)

    for _ in range(n_therm):
        key, sweep_key = jax.random.split(key)
        links = sweep(links, beta, sweep_key)

    plaquette_samples: list[float] = []
    polyakov_samples: list[float] = []
    operator_samples: list[jnp.ndarray] = []
    gevp_operator_samples: list[jnp.ndarray] = []
    ...
    for _ in range(n_meas):
        ...
        gevp_operator_samples.append(jnp.stack(gevp_ops))
    ...
    o_samples = jnp.stack(operator_samples)
    ...
    gevp_o = jnp.stack(gevp_operator_samples)
