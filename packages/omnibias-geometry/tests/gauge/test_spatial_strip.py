...
"""Spatial-strip transfer: gap, reflection positivity, cluster tail."""
...
def test_strip_is_16_dimensional_and_symmetric() -> None:
    transfer = su2_spatial_strip_transfer(STRIP_COUPLING_LOCK, n_sites=2, n_angles=4)
...
def test_strip_gap_is_certified() -> None:
    transfer = su2_spatial_strip_transfer(STRIP_COUPLING_LOCK, n_sites=2, n_angles=4)
...
def test_reflection_positivity_forms_are_non_negative() -> None:
    transfer = su2_spatial_strip_transfer(STRIP_COUPLING_LOCK, n_sites=2, n_angles=4)
...
def test_cluster_tail_contains_a_numerical_sample() -> None:
    transfer = su2_spatial_strip_transfer(STRIP_COUPLING_LOCK, n_sites=2, n_angles=4)
...
def test_rp_seal_replay_and_honesty() -> None:
    transfer = su2_spatial_strip_transfer(STRIP_COUPLING_LOCK, n_sites=2, n_angles=4)
    result = certified_strip_reflection_positivity(transfer)
...
def test_rebuild_and_transfer_gap_machine() -> None:
    transfer = su2_spatial_strip_transfer(STRIP_COUPLING_LOCK, n_sites=2, n_angles=4)
    ...
    rp = machine.evaluate(
        Conjecture(
            "strip rp",
            STRIP_RP_KIND,
            {"coupling": STRIP_COUPLING_LOCK, "n_sites": 2, "n_angles": 4},
        )
    )
