...
def _wrap_ledger(body: str) -> str:
    return _LEDGER_HEADER + body + _FOOTER


def _wrap_cone(body: str) -> str:
    return _CONE_HEADER + body + _FOOTER
...
def _gen_convergence_ledger(cert: Mapping[str, Any]) -> str | None:
    """Apply a Check convergence-ledger theorem, re-derived over ``Q``."""
    payload = cert.get("payload")
    if not (isinstance(payload, Mapping) and payload.get("type") == "convergence_ledger"):
        return None
    if not bool(payload.get("holds")):
        return None
    family = payload.get("family") or payload.get("name")
    ...
    if family == "navier_stokes_exponent":
        if not check_ledger(navier_stokes_exponent_ledger()).holds:
            return None
        comment = (
            "Navier-Stokes exponent-ledger margins at the manuscript "
            "parameters (finite rational algebra; not Clay A/B)."
        )
        stmt = (
        ...
        thm = "ns_manuscript_margins"
    elif family == "strong_coupling_polymer":
        if not check_ledger(strong_coupling_polymer_ledger()).holds:
            return None
        comment = (
            "strong-coupling polymer majorants 15 < 20 and 15 < 24 "
            "(finite arithmetic; not a continuum gauge claim)."
        )
        stmt = (
        ...
        thm = "polymer_ledger_margins"
    elif family == "navier_stokes_scale":
        from omnibias.core.proof.obligations.convergence_ledger import (
            navier_stokes_scale_ledger,
        )

        if not check_ledger(navier_stokes_scale_ledger()).holds:
            return None
        comment = (
            "Navier-Stokes scale-ledger geometry / energy bounds at "
            "h = 1/200 (finite rational algebra; not a forced-blowup reproof)."
        )
        stmt = "(1 / 200 : ℚ) < 1 / 100 ∧ (1 / 200 : ℚ) < 1 / 6"
        thm = "ns_scale_margins"
    else:
        return None
    body = (
        f"/-- {comment} -/\n"
    ...
    return _wrap_ledger(body)


def _gen_stress_cone(cert: Mapping[str, Any]) -> str | None:
    """Apply the locked interior-cone theorem, re-derived over ``Q``."""
    payload = cert.get("payload")
    ...
    if not check_cone(locked_interior_cone()).holds:
        return None
    comment = (
        "admissible-stress interior cone at T=(1,1) against e1, e2 "
        "(finite rational algebra; not a forced-blowup reproof)."
    )
    body = (
        f"/-- {comment} -/\n"
        "theorem obligation :\n"
        ...
        "      0 < coneLambda2 1 0 1 1 1 :=\n"
        "  locked_cone_interior\n"
    )
    return _wrap_cone(body)
...
_GENERATORS: tuple[tuple[str, Callable[[Mapping[str, Any]], str | None]], ...] = (
    ...
    ("convergence_ledger", _gen_convergence_ledger),
    ("stress_cone", _gen_stress_cone),
    ("sign", _gen_sign),
)
...
def generate_obligation(cert: Mapping[str, Any]) -> str | None:
    ...
    a named convergence-ledger instance,
    a named stress-cone instance,
...
def classify_obligation(cert: Mapping[str, Any]) -> str | None:
    ...
    ``"sixj"``, ``"haar_volume"``, ``"convergence_ledger"``, ``"stress_cone"``
    or ``"sign"`` -- whichever
