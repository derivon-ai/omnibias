...
def generate_obligation(cert: Mapping[str, Any]) -> str | None:
    ...
    polynomial, the Krawczyk test, then the enclosed-quantity sign (interval /
    PDE finite margin / Taylor-model centre value / raw ``lo``/``hi`` mapping).
    Every emitted obligation is first re-derived exactly over ``ℚ``; a payload
    that does not reproduce a sign-definite / contracting fact yields ``None``.
    """
...
def classify_obligation(cert: Mapping[str, Any]) -> str | None:
    ...
    One of ``"positive_definite"``, ``"radii_polynomial"``, ``"krawczyk"`` or
    ``"sign"`` -- whichever generator first yields Lean, re-derived exactly over
    ``ℚ`` -- or ``None`` when the certificate carries no Mathlib-checkable finite
    obligation.  Runs no Lean: it is the cheap triage the loop driver uses before
