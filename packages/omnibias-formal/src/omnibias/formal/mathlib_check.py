...
2. emits a tiny Lean source file (``OmnibiasAnalytic/Generated.lean``) that
   ...
   Check unique-root theorem for planted NK existence, and by closing the
   concrete rational (in)equalities with ``norm_num`` for the rest.  Every
...
def generate_obligation(cert: Mapping[str, Any]) -> str | None:
    ...
    existence, then the enclosed-quantity sign (interval / PDE finite margin /
    Taylor-model centre value / raw ``lo``/``hi`` mapping).  Every emitted
...
def classify_obligation(cert: Mapping[str, Any]) -> str | None:
    ...
    ``"tower_coeffs"``, ``"nk_existence"`` or ``"sign"`` -- whichever
