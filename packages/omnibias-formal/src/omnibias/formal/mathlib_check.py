...
2. emits a tiny Lean source file (``OmnibiasAnalytic/Generated.lean``) that
   ...
   enclosure-trace plant theorem, and by closing the concrete rational
   (in)equalities with ``norm_num`` for the rest.  Every rational is emitted
   **directly** (no common-denominator integer scaling);
...
def generate_obligation(cert: Mapping[str, Any]) -> str | None:
    ...
    existence, a planted enclosure trace, then the enclosed-quantity sign
    (interval / PDE finite margin / Taylor-model centre value / raw ``lo``/``hi``
    mapping).  Every emitted
...
def classify_obligation(cert: Mapping[str, Any]) -> str | None:
    ...
    ``"tower_coeffs"``, ``"nk_existence"``, ``"enclosure_trace"`` or
    ``"sign"`` -- whichever
