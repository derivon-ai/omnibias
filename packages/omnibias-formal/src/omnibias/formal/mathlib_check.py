...
2. emits a tiny Lean source file (``OmnibiasAnalytic/Generated.lean``) that
   ...
   theorem, and by closing the concrete rational (in)equalities with
   ``norm_num`` for the rest.  Every rational is emitted **directly**
   (no common-denominator integer scaling);
...
def generate_obligation(cert: Mapping[str, Any]) -> str | None:
    ...
    existence, a planted enclosure trace, a named unique-zero plant, then
    the enclosed-quantity sign (interval / PDE finite margin / Taylor-model
    centre value / raw ``lo``/``hi`` mapping).  Every emitted
