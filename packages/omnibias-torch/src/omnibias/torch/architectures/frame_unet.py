...
def frame_unet_forward(
    ...
    *,
    ...
    amps: Sequence[float] | Tensor | None = None,
    seed: int = 0,
    ...
    if amps is None:
        coef = None
    elif isinstance(amps, Tensor):
        coef = [float(v) for v in amps.reshape(-1).tolist()]
    else:
        coef = [float(v) for v in amps]
    ys: list[float] = []
    ...
    for xi in xs:
        ...
        ys.append(y)
        last = skips
    return x.new_tensor(ys).reshape(x.shape), last
