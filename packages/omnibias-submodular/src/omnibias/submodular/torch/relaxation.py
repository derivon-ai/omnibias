...
r"""Differentiable continuous-greedy relaxation (torch) on the multilinear extension.

Bit-identical twin of :mod:`omnibias.submodular.jax.relaxation` (float64). Continuous
...
unrolled through ``autograd``, so a model that predicts the coverage data trains
...
``docs/theory.md`` and :mod:`omnibias.torch.unit`).
"""
...
MultilinearFn = Callable[[Tensor], Tensor]


def coverage_multilinear(p: object, membership: object, weights: object) -> Tensor:
    ...
    """
    c = torch.as_tensor(membership, dtype=torch.float64)
    w = torch.as_tensor(weights, dtype=torch.float64)
    pv = torch.as_tensor(p, dtype=torch.float64)
    if pv.ndim == 1:
        uncovered = torch.prod(1.0 - c * pv[None, :], dim=1)
        return torch.sum(w * (1.0 - uncovered))
    a = 1.0 - c[None, :, :] * pv[:, None, :]
    uncovered_b = torch.prod(a, dim=2)
    return torch.sum(w[None, :] * (1.0 - uncovered_b), dim=1)


def _soft_basis(g: Tensor, groups: list[Tensor], caps: list[int], beta: float) -> Tensor:
    n = int(g.shape[0])
    ...
    for group, cap_raw in zip(groups, caps, strict=True):
        ...
        if cap <= 0:
            continue
        if cap >= size:
            y = y.index_copy(0, group, torch.ones(size, dtype=torch.float64))
            continue
        gg = g[group]
        sorted_desc = torch.sort(gg, descending=True).values
        tau = 0.5 * (sorted_desc[cap - 1] + sorted_desc[cap])
        y = y.index_copy(0, group, torch.sigmoid(beta * (gg - tau)))
    return y
...
def continuous_greedy(
    ...
    n = matroid.n
    groups = [torch.as_tensor(g, dtype=torch.long) for g in matroid.groups()]
    caps = matroid.caps()
...
def coverage_relaxation(
    ...
    Pass torch tensors to differentiate the fractional ``p*`` through the coverage data.
    """
    c = torch.as_tensor(membership, dtype=torch.float64)
    w = torch.as_tensor(weights, dtype=torch.float64)

    def fn(p: Tensor) -> Tensor:
        return coverage_multilinear(p, c, w)

    return continuous_greedy(fn, matroid, schedule)


def facility_multilinear(p: object, gains: object, weights: object) -> Tensor:
    ...
    """
    g = torch.as_tensor(gains, dtype=torch.float64)  # (m, n)
    w = torch.as_tensor(weights, dtype=torch.float64)  # (m,)
    m = int(g.shape[0])
    order = torch.as_tensor(np.argsort(-np.asarray(gains, dtype=float), axis=1), dtype=torch.long)
    v_sorted = torch.take_along_dim(g, order, dim=1)  # (m, n) descending gains
    pv = torch.as_tensor(p, dtype=torch.float64)
    if pv.ndim == 1:
        q = pv[order]  # (m, n) probabilities in sorted-gain order
        incl = torch.cumprod(1.0 - q, dim=1)
        surv = torch.cat([torch.ones((m, 1), dtype=torch.float64), incl[:, :-1]], dim=1)
        emax = torch.sum(v_sorted * q * surv, dim=1)  # (m,)
        return torch.sum(w * emax)
    q = pv[:, order]  # (B, m, n)
    incl = torch.cumprod(1.0 - q, dim=2)
    surv = torch.cat([torch.ones((pv.shape[0], m, 1), dtype=torch.float64), incl[:, :, :-1]], dim=2)
    emax = torch.sum(v_sorted[None, :, :] * q * surv, dim=2)  # (B, m)
    return torch.sum(w[None, :] * emax, dim=1)


def _budget_dp(
    pv: Tensor, support: Tensor, src: Tensor, capped: Tensor, zero_index: int
) -> Tensor:
    ...
    the arithmetic is identical to the jax twin.
    """
    length = int(support.shape[0])
    n = int(src.shape[0])
    init = (torch.arange(length) == zero_index).to(torch.float64)
    if pv.ndim == 1:
        prob = init
        for i in range(n):
            si = src[i]
            mask = (si >= 0).to(torch.float64)
            gathered = prob[torch.clamp(si, 0, length - 1)] * mask
            prob = prob * (1.0 - pv[i]) + gathered * pv[i]
        return torch.sum(prob * capped)
    batch = int(pv.shape[0])
    prob = init.unsqueeze(0).repeat(batch, 1)
    for i in range(n):
        si = src[i]
        idx = torch.clamp(si, 0, length - 1)
        mask = (si >= 0).to(torch.float64)
        gathered = prob[:, idx] * mask[None, :]
        pi = pv[:, i][:, None]
        prob = prob * (1.0 - pi) + gathered * pi
    return torch.sum(prob * capped[None, :], dim=1)


def budget_multilinear(p: object, ground: object, budget: object) -> Tensor:
    ...
    support = torch.as_tensor(support_np, dtype=torch.float64)
    src = torch.as_tensor(src_np, dtype=torch.long)
    capped = torch.minimum(support, torch.as_tensor(budget, dtype=torch.float64))
    return _budget_dp(torch.as_tensor(p, dtype=torch.float64), support, src, capped, zero_index)


def graphcut_multilinear(p: object, weights: object) -> Tensor:
